(fn [state time-left]
  ...
  (defn pick-move
      ...
      [arena self]
      (if (can-shoot-enemy? (get-direction arena) (add-locs arena) arena-size)
          (build-resp :shoot)
          (if (empty? (filter-arena (focus-sight arena) "food"))
              (if (can-shoot-barrier? (get-direction arena) (add-locs arena) arena-size)
                  (build-resp :shoot)
    ...
    {:command (pick-move (:arena state) {:x 3 :y 3})
     :state {:move (pick-move (:arena state) {:x 3 :y 3})
             ...
             :shootable (can-shoot-enemy? (get-direction (:arena state)) (add-locs (:arena state)) arena-size)
